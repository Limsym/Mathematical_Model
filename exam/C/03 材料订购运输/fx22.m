function [Nend,result,kk]=fx22(x0)
...
% nt=size(chooset2,1);
% x0=chooset2(nt,:)';
% choose0=index{7,5}(x0,1);
% for ii=1:5;chooseo(:,ii)=[index{7,5}(chooset2(ii,:)',1)]';end
obj=Prod(x0,:);
ppm=[0.6 0.66 0.72];
for ii=1:size(x0,2)
	choose0(ii)=find(index{7,5}(:,1)==x0(ii));
end
...
for i=1:3
    id=intersect(find(index{1}(:,1)==i),x0);
    num=size(id,1);
    for j=1:num
        ca{i}(j)=find(x0==id(j));
    end
...
ar=3;
ys=0;kk=[ones(ar,1)*0.6 ones(ar,2)*0.2];
ddd=0.02;
while ys==0
    for j=1:m
        for i=1:n
