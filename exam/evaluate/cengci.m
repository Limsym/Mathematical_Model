%% updated at 0516
function textmaxP=cengci(A,B1,B2,B3,B4,B5)
% 层次分析赋权法
%% calculate the lmbda=maximum eigenvalue 
[VA,DA]=eig(A);
...
lambdaB1=DB1(sB1,tB1);
    ...
    if CRB1<=CRlB1
        text1B1=['CRB1<=CRl   √    ',num2str(CRB1)];
    else text1B1=['CRB1>CRl   ×    ',num2str(CRB1)];
    end
    disp(text1B1)
...
lambdB2a=DB2(sB2,tB2);
    ...
    if CRB2<=CRlB2
        text1B2=['CRB2<=CRl   √    ',num2str(CRB2)];
    else text1B2=['CRB2>CRl   ×    ',num2str(CRB2)];
    end
    disp(text1B2)
%Eigenvector & guiyihuachuli
eigenvectorB2=V(:,sB2);
