...
pc_dot=(dot-(unknown+n))/dot;                   % dot偏差，检验算法是否可行
for i=1:dot
    t(i)=(i-1)*dt;
    x(i)=(i-1)*dt*v;                            % x(i)=x(i-1)+v*dt;
    for wq=1:nw
        if  x(i)<=xb
            p(i)=1;
            l(i)=0;
...
figure;grid on;hold on;scatter(t(nowt:dot),ET(nowt:dot));scatter(t(nowt:dot),T(nowt:dot))
end
%% k的分区拟合
for i=1:dot
    newdot=355;
...
if paint==1
    ...
    scatter(t(39:746),pT(39:746),5,'r','filled');xlabel('时间(s)');ylabel('T(℃)');hold on
    scatter(t(39:746),Ta(39:746),5,'r','filled');xlabel('时间(s)');ylabel('T(℃)');hold on
    scatter(t(39:746), T(39:746),5,'b','filled');xlabel('时间(s)');ylabel('T(℃)');grid on;
    legend('拟合曲线','实验曲线');
    paint=0;
end